...
if not web3.is_connected():
    sys.exit("Error: Unable to connect to Arbitrum Sepolia network.")
...
def extract_transaction_details(user_input):
    """ Extracts transaction details only if 'Arbitrum' is mentioned. """
    if "arbitrum" not in user_input.lower():
        return None
    ...
    try:
        ...
        **Response Format (strict JSON only, no extra text, no markdown formatting):**
        ...
        # Clean response by removing markdown-style backticks if present
        clean_text = response.text.strip().strip("```json").strip("```").strip()

        try:
            result = json.loads(clean_text)
        except json.JSONDecodeError:
            print(f"AI Response Error: Not valid JSON → {clean_text}")
            return None
...
def wait_for_confirmation(web3, tx_hash, max_retries=20):
        ...
        try:
            ...
            if tx_receipt:
                if tx_receipt.status == 1:
                    ...
                    print(f"⚠️ Transaction Failed! Hash: {tx_hash.hex()}")
                return tx_receipt
        except Exception:
    ...
    print(f"⚠️ Transaction not found after {max_retries * 5} seconds. Check manually:")
    print(f"🔗 Explorer: https://sepolia.arbiscan.io/tx/{tx_hash.hex()}")
    return None

def send_transaction(to_address, amount):
    """ Sends a signed transaction on Arbitrum Sepolia. """
    try:
